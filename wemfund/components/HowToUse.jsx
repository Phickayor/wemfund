import React from "react";

function HowToUse() {
  return (
    <div className="flex font-[poppins] text-darkblue md:h-fit h-96 my-10 mx-auto md:w-10/12 w-11/12">
      <div className="flex bg-purple ">
        <img src="/howToUseImage.png" className="self-center" />
      </div>
      <div className="flex flex-col md:justify-end md:py-10 py-5 md:w-full ">
        <h1 className="text-center font-bold md:text-6xl text-black my-5">
          How TripPayer works
        </h1>
        <div className="space-y-8 py-6 mx-auto text-darkblue overflow-y-scroll md:overflow-y-visible">
          <div className=" md:mx-auto mx-4 space-y-2 md:w-9/12">
            <div className="flex md:space-x-10 space-x-5 text-md md:text-3xl font-semibold ">
              <h1 className="font-bold">01</h1>
              <h1 className="font-semibold">Create an Account</h1>
            </div>
            <p className="md:w-9/12 md:mx-3 md:px-12 pl-6 text-left border-l-4 md:text-xl text-xs">
              Sign up for an account with your name, email and phone
              number.
            </p>
          </div>
          <div className=" md:mx-auto mx-4 space-y-2 md:w-9/12">
            <div className="flex md:space-x-10 space-x-5 md:text-3xl font-semibold ">
              <h1 className="font-bold">02</h1>
              <h1 className="font-semibold">Set Up Your Profile</h1>
            </div>
            <p className=" md:w-9/12 md:mx-3 md:px-12 pl-6 text-left border-l-4 md:text-xl text-xs">
              Sign up for an account with your name, email and phone
              number.
            </p>
          </div>
          <div className=" md:mx-auto mx-4 space-y-2 md:w-9/12">
            <div className="flex md:space-x-10 space-x-5 md:text-3xl font-semibold ">
              <h1 className="font-bold">03</h1>
              <h1 className="font-semibold">Pay Ride</h1>
            </div>
            <p className=" md:w-9/12 md:mx-3 md:px-12 pl-6 text-left border-l-4 md:text-xl text-xs">
              Sign up for an account with your name, email and phone
              number.
            </p>
          </div>
          <div className=" md:mx-auto mx-4 space-y-2 md:w-9/12">
            <div className="flex md:space-x-10 space-x-5 md:text-3xl font-semibold ">
              <h1 className="font-bold">04</h1>
              <h1 className="font-semibold">View Transaction History</h1>
            </div>
            <p className="md:w-9/12 md:mx-3 md:px-12 pl-6 text-left border-l-4 md:text-xl text-xs">
              Sign up for an account with your name, email and phone
              number.
            </p>
          </div>
          {/* <div className=" mx-auto  space-y-2">
            <div className="flex space-x-10 text-3xl font-semibold">
              <h1 className="font-bold">02</h1>
              <h1>Set Up Your Profile</h1>
            </div>
            <p className="mx-3 px-12 text-left border-l-4 text-xl">
              Sign up for an account with <br /> your name, email and phone
              number.
            </p>
          </div>
          <div className=" mx-auto space-y-2">
            <div className="flex space-x-10 text-3xl font-semibold">
              <h1 className="font-bold">03</h1>
              <h1>Pay Ride </h1>
            </div>
            <p className="mx-3 px-12 text-left border-l-4 text-xl">
              Sign up for an account with <br /> your name, email and phone
              number.
            </p>
          </div>
          <div className=" mx-auto  space-y-2">
            <div className="flex space-x-10 text-3xl font-semibold">
              <h1 className="font-bold">04</h1>
              <h1>View Transaction History</h1>
            </div>
            <p className=" mx-3 px-12 text-left border-l-4 text-xl">
              Sign up for an account with <br /> your name, email and phone
              number.
            </p>
          </div> */}
        </div>
      </div>
    </div>
  );
}

export default HowToUse;
